...
function drawText(text, x, y, angle) {
    ...
    const gradient = ctx.createLinearGradient(-textWidth/2, -10, textWidth/2, 10);
    gradient.addColorStop(0, 'rgba(0, 0, 0, 0.4)');
    gradient.addColorStop(1, 'rgba(0, 0, 0, 0.2)');
    
    ctx.fillStyle = gradient;
    ctx.fillRect(-textWidth/2 - padding, -10, textWidth + padding*2, 20);
...
function drawWheel() {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        const textRadius = radius * 0.5; // Metni daha içeri al
...
function spinWheel() {
    ...
    function animate(currentTime) {
        ...
        if (progress < 1) {
            ...
            // Kazanan dilimi hesapla (üstteki işaretçiye göre)
            const normalizedRotation = currentRotation % (2 * Math.PI);
            const segmentAngle = (2 * Math.PI) / segments.length;
            // İşaretçi üstte olduğu için, seçilen dilim işaretçinin altındaki dilim olacak
